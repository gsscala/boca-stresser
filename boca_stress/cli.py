...
def setup(
    ...
    async def _setup() -> None:
        async with BocaAdmin(url) as admin:
            admin.set_admin_credentials(admin_user, admin_pass)
            ...
            if prob_path.exists():
                zips = sorted(list(prob_path.glob("*.zip")))
                for i, zip_file in enumerate(zips, start=1):
                    console.print(f"Uploading problem {zip_file.name} (#{i})...")
                    success = await admin.upload_problem(zip_file, i)
                    if success:
            ...
            success = await admin.import_teams(teams)
            if success:
